...
This module implements one deliberately narrow, post-hoc external-dataset
directional corroboration of the Li et al. 2022 Lac-Phe exercise claim using
Metabolomics Workbench study ST003662. Cohort independence is not established,
and the selected dataset is exploratory rather than a prespecified replication.
It does not reproduce the paper's datasets or its causal claims.
...
CASE_SCOPE = "post_hoc_external_dataset_directional_corroboration"
SUPPORTED_OUTCOME = "supports_directional_claim_requires_identity_review"
...
def _validate_contract(contract: Any) -> dict[str, Any]:
    ...
    if "sampling delay" not in timing_caveat or "Before/After" not in timing_caveat:
        raise BiomarkerReproductionInputError(
            "replication.timing_caveat must preserve the unresolved Before/After "
            "sampling-delay limitation"
        )
...
def _validate_and_compute_pairs(
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if pre is not None and post is not None and pre > 0.0 and post > 0.0:
            # Taking the logs separately avoids ratio overflow/underflow for valid,
            # finite positive measurements near the limits of binary64.
            log2_changes.append(math.log2(post) - math.log2(pre))
    ...
    positive_count = sum(change > 0.0 for change in log2_changes)
    negative_count = sum(change < 0.0 for change in log2_changes)
    tie_count = len(log2_changes) - positive_count - negative_count
...
def _validate_cached_effect(
    ...
    neg_log10_p_difference: float | None
    neg_log10_p_consistent: bool | None
    if numbers["p_value"] > 0.0:
        neg_log10_p_difference = abs(numbers["neg_log10_p"] - (-math.log10(numbers["p_value"])))
        ...
        neg_log10_p_difference = None
        neg_log10_p_consistent = None
    return {
...
def _outcome_and_checks(
    ...
    checks = {
        ...
        "exact_sign_test_p_value_within_threshold": (
            sign_p is not None and sign_p <= rules["maximum_exact_sign_test_p_value"]
        ),
    ...
    if not checks["minimum_complete_positive_pairs_met"] or sign_p is None:
...
def _build_result(
    ...
    return {
        ...
        "interpretation": (
            "This is a post-hoc external-dataset directional corroboration in "
            "ST003662, not an exact reproduction of the Li et al. datasets or "
            "causal feeding/obesity findings. Dataset selection was exploratory, "
            "cohort independence is not established, metabolite identity still "
            "requires human review, and whole-blood comparability plus exercise "
            "sampling timing remain unresolved."
        ),
    }
...
def _render_report(result: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Workbench dataset shows the same *direction* as the 2022 Li et al. claim "
        "that circulating Lac-Phe increases after exercise. Cohort independence is "
        "not established. It does not use the paper's datasets and does not reproduce "
        "its causal feeding or obesity results.\n\n"
        "## Evidence contract\n\n"
...
def evaluate_biomarker_reproduction(
    ...
    """Evaluate a checksum-bound, post-hoc Lac-Phe directional corroboration case.
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the offline Li et al. Lac-Phe post-hoc external-dataset "
            "directional corroboration case."
        )
    )
