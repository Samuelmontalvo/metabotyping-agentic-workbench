...
def human_review_reason(row: dict[str, str]) -> str:
    ...
    if variable == "sex" and "distinct constructs" in evidence:
        return (
            "Confirm whether the source variable records biological sex or gender identity; "
            "the two are distinct constructs and are not interchangeable."
        )
