...
description: Use when assembling exercise-phenotype construct, protocol, unit, and derivation evidence into an advisory review packet.
---

# Purpose
Inventory what a study reports about how an exercise phenotype was defined, measured, and derived, without accepting a mapping or a unit transform.

# Inputs
- Schema-valid variable cards and codebooks with resolvable provenance locators.
- Provenance-linked evidence for every required dimension: `construct`, `codebook_definition`, `protocol`, `exercise_mode`, `device_analyzer_and_calibration`, `endpoint`, `maximality_criteria`, `unit`, `normalization_basis`, `derivation_rules`, `assessment_timing`, `actigraphy_processing`, `repeated_trial_handling`, `training_status`.
...
# Outputs
- An advisory `DomainReviewPacket` covering exercise-phenotype construct and protocol evidence, with linked limitations, blockers, adjudication questions, and a deterministic digest.
...
- Matching names or units do not establish phenotype equivalence; VO2max and VO2peak stay distinct without endpoint evidence.
...
- Critical evidence: never collapse measured, estimated, device-derived, and self-reported phenotypes, and never treat a shared label as a shared construct; do not record an inferred value and do not infer an evidence state from silence.
