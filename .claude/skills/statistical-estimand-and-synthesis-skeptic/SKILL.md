...
description: Use when assembling estimand, model, uncertainty, and synthesis-dependency evidence into an advisory review packet.
---

# Purpose
Inventory what a reported effect actually estimates and what would have to be true before any synthesis, without calculating or pooling an effect.

# Inputs
- Schema-valid study-specific effect summaries with resolvable provenance locators.
- Provenance-linked evidence for every required dimension: `estimand`, `population`, `intervention_or_exposure`, `comparator`, `outcome`, `timepoint`, `effect_scale`, `effect_orientation`, `model`, `covariates`, `repeated_measures_or_clustering`, `uncertainty`, `sample_counts`, `missing_data_handling`, `multiplicity`, `dependencies`, `upstream_mapping_status`, `upstream_assay_status`, `heterogeneity_plan`, `sensitivity_plan`.
...
# Outputs
- An advisory `DomainReviewPacket` covering estimand and synthesis-readiness evidence, with linked limitations, blockers, adjudication questions, and a deterministic digest.
...
- Two effects that share an outcome name can answer different questions; an upstream planning flag is not authorization to synthesize.
...
- Critical evidence: never compute an effect, choose a model, pool an estimate, or return a meta-analytic result; do not record an inferred value and do not infer an evidence state from silence.
